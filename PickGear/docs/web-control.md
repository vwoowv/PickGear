...
| `RESTART_GAME` | 같은 게임 종류로 점수 초기화 후 1라운드부터 다시 시작 | 일시정지 또는 정상 종료 |
...
`state.audio`에는 음악 재생 요청 여부(`requested`), 실제 재생 상태(`playing`), 일시정지 여부(`paused`), 재생 시간(`currentTime`), 음량(`volume`), 브라우저 오디오 활성화 여부(`browserUnlocked`)가 포함됩니다. 소리가 안 날 때 `GET_GAME_STATE` 응답으로 확인할 수 있습니다.

`status`: `title`, `playing`, `paused`, `ended`, `exited`.
`sequence`: `Prepare`, `ShowSuit`, `GameRound`, `Result`, `EndGame`, `Exited`.
`elapsedSeconds`는 현재 진행 단계의 경과 시간입니다. `gameType`은 YG=0, JYP=1, SM=2, HYBE=3입니다. `GAME_PAUSED.state.score`로 일시정지 창 점수를 표시하세요. `GAME_EXITED`의 점수는 정리 후 0입니다. 중도 종료 전 점수가 필요하면 일시정지 응답을 보관하세요.
...
게임 런타임의 `gameInstance.I.playing`에는 다음 public 메서드가 있습니다. 전역 `window` 객체에 별도 API를 노출하지 않으므로 외부 웹은 postMessage를 사용하세요.
...
- 프로젝트 루트에서 `python3 -m http.server 8766 --bind 127.0.0.1 --directory tests` 실행 후 `http://127.0.0.1:8766/web-control.html`을 엽니다.
